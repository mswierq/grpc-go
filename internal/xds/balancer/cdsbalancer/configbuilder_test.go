	...
	gotConfig, _, err := buildAggregateClusterConfigJSON([]*priorityConfig{
		{
			...
			childNameGen: newNameGenerator(testClusterName),
		},
		{
			clusterConfig: &xdsresource.ClusterConfig{
				Cluster: &xdsresource.ClusterUpdate{
			...
			childNameGen: newNameGenerator(testClusterName2),
		},
	}, nil)
	if err != nil {
...
func (s) TestBuildAggregateClusterConfig(t *testing.T) {
	...
	p0Config := noopODCfg
	p0Config.ChildPolicy = &iserviceconfig.BalancerConfig{
			...
			ChildPolicy: &iserviceconfig.BalancerConfig{
				...
				Config: &priority.LBConfig{
					Children: map[string]*priority.Child{
						fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName): {
							Config:                     nil,
							IgnoreReresolutionRequests: true,
						},
						fmt.Sprintf("{cluster=%s, child_number=1}", testClusterName): {
							Config:                     nil,
							IgnoreReresolutionRequests: true,
						},
					},
					Priorities: []string{fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), fmt.Sprintf("{cluster=%s, child_number=1}", testClusterName)},
				},
			},
		},
	}
	...
	p1Config := noopODCfg
	p1Config.ChildPolicy = &iserviceconfig.BalancerConfig{
		Name: clusterimpl.Name,
		Config: &clusterimpl.LBConfig{
			...
			ChildPolicy: &iserviceconfig.BalancerConfig{
				...
				Config: &priority.LBConfig{
					Children: map[string]*priority.Child{
						fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName2): {
							Config:                     nil,
							IgnoreReresolutionRequests: false,
						},
					},
					Priorities: []string{fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName2)},
				},
			},
		},
	}
	...
	wantConfig := &priority.LBConfig{
		...
		Priorities: []string{testClusterName, testClusterName2},
	}
	if diff := cmp.Diff(gotConfig, wantConfig); diff != "" {
		t.Errorf("buildAggregateClusterConfig() diff (-got +want) %v", diff)
	}
}
...
func (s) TestBuildLeafClusterConfigJSON(t *testing.T) {
	...
	gotConfig, _, err := buildLeafClusterConfigJSON(&priorityConfig{
		...
		outlierDetection: noopODCfg,
		childNameGen:     newNameGenerator(clusterName),
...
func (s) TestBuildLeafClusterConfig_DNS(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			wantEndpoints := []resolver.Endpoint{testEndpointForDNS(tt.endpoints, 1, []string{fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName2), xdsinternal.LocalityString(clients.Locality{})})}
...
func (s) TestBuildLeafClusterConfig_EDS_PickFirstWeightedShuffling_Disabled(t *testing.T) {
	...
	wantEndpoints := []resolver.Endpoint{
		testEndpointWithAttrs(loc0.Endpoints[0].ResolverEndpoint, 20, 20*1, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &loc0.ID),
		testEndpointWithAttrs(loc0.Endpoints[1].ResolverEndpoint, 20, 20*1, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &loc0.ID),
		testEndpointWithAttrs(loc1.Endpoints[0].ResolverEndpoint, 80, 80*1, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &loc1.ID),
		testEndpointWithAttrs(loc1.Endpoints[1].ResolverEndpoint, 80, 80*1, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &loc1.ID),
		testEndpointWithAttrs(loc2.Endpoints[0].ResolverEndpoint, 20, 20*1, fmt.Sprintf("{cluster=%s, child_number=1}", testClusterName), &loc2.ID),
		testEndpointWithAttrs(loc2.Endpoints[1].ResolverEndpoint, 20, 20*1, fmt.Sprintf("{cluster=%s, child_number=1}", testClusterName), &loc2.ID),
		testEndpointWithAttrs(loc3.Endpoints[0].ResolverEndpoint, 80, 80*1, fmt.Sprintf("{cluster=%s, child_number=1}", testClusterName), &loc3.ID),
		testEndpointWithAttrs(loc3.Endpoints[1].ResolverEndpoint, 80, 80*1, fmt.Sprintf("{cluster=%s, child_number=1}", testClusterName), &loc3.ID),
	}
...
func (s) TestBuildLeafClusterConfig_EDS_PickFirstWeightedShuffling_Enabled(t *testing.T) {
	...
	wantEndpoints := []resolver.Endpoint{
		testEndpointWithAttrs(loc0.Endpoints[0].ResolverEndpoint, 20, 214748364, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &loc0.ID),
		testEndpointWithAttrs(loc0.Endpoints[1].ResolverEndpoint, 20, 214748364, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &loc0.ID),
		testEndpointWithAttrs(loc1.Endpoints[0].ResolverEndpoint, 80, 858993459, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &loc1.ID),
		testEndpointWithAttrs(loc1.Endpoints[1].ResolverEndpoint, 80, 858993459, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &loc1.ID),
		testEndpointWithAttrs(loc2.Endpoints[0].ResolverEndpoint, 20, 214748364, fmt.Sprintf("{cluster=%s, child_number=1}", testClusterName), &loc2.ID),
		testEndpointWithAttrs(loc2.Endpoints[1].ResolverEndpoint, 20, 214748364, fmt.Sprintf("{cluster=%s, child_number=1}", testClusterName), &loc2.ID),
		testEndpointWithAttrs(loc3.Endpoints[0].ResolverEndpoint, 80, 858993459, fmt.Sprintf("{cluster=%s, child_number=1}", testClusterName), &loc3.ID),
		testEndpointWithAttrs(loc3.Endpoints[1].ResolverEndpoint, 80, 858993459, fmt.Sprintf("{cluster=%s, child_number=1}", testClusterName), &loc3.ID),
	}
...
func (s) TestBuildLeafClusterConfig_NilChecks(t *testing.T) {
	tests := []struct {
		name string
		p    *priorityConfig
	}{
		...
		},
		{
			...
			p:    &priorityConfig{childNameGen: newNameGenerator(clusterName)},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, _, err := buildLeafClusterConfig(tt.p, nil)
			if err == nil {
				t.Errorf("buildLeafClusterConfig() succeeded unexpectedly, want error")
			}
		})
	}
}
...
func (s) TestPriorityLocalitiesToClusterImpl_HTTP11Proxy(t *testing.T) {
	...
	}{
		{
			...
			wantEndpoints: []resolver.Endpoint{
				testEndpointWithAttrs(
					...
					1, 2147483648, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &clients.Locality{SubZone: "testzone-1"},
				),
			},
		...
		},
		{
			...
			wantEndpoints: []resolver.Endpoint{
				testEndpointWithAttrs(
					...
					1, 2147483648, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &clients.Locality{SubZone: "subzone-1"},
				),
			},
			wantConnectAddr: []string{"10.0.0.5:443"},
		},
		...
		},
		{
			...
			wantEndpoints: []resolver.Endpoint{
				testEndpointWithAttrs(
					...
					1, 1073741824, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &clients.Locality{SubZone: "subzone-1"},
				),
				testEndpointWithAttrs(
					resolver.Endpoint{
					...
					1, 1073741824, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &clients.Locality{SubZone: "subzone-2"},
				),
			},
		...
		},
		{
			...
			wantEndpoints: []resolver.Endpoint{
				testEndpointWithAttrs(
					...
					1, 536870912, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &clients.Locality{SubZone: "subzone-1"},
				),
				testEndpointWithAttrs(
					resolver.Endpoint{
						Addresses: []resolver.Address{
				...
				testEndpointWithAttrs(
					...
					1, 536870912, fmt.Sprintf("{cluster=%s, child_number=0}", testClusterName), &clients.Locality{SubZone: "subzone-2"},
				),
				testEndpointWithAttrs(
					resolver.Endpoint{
						Addresses: []resolver.Address{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, retEndpoints, err := priorityLocalitiesToClusterImpl(tt.localities, fmt.Sprintf("{cluster=%s, child_number=0}", tt.clusterUpdate.ClusterName), tt.clusterUpdate, nil)
			if err != nil {
