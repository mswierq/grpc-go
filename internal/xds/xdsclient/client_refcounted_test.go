...
func (s) TestClientNew_Single(t *testing.T) {
	...
	_, closeFunc, err := pool.NewClient(t.Name(), &estats.UnimplementedMetricsRecorder{}, nil)
	if err != nil {
	...
	for i := 0; i < count; i++ {
		func() {
			_, closeFuncs[i], err = pool.NewClient(t.Name(), &estats.UnimplementedMetricsRecorder{}, nil)
			if err != nil {
...
func (s) TestClientNew_Multiple(t *testing.T) {
	...
	_, closeFunc1, err := pool.NewClient(client1Name, &estats.UnimplementedMetricsRecorder{}, nil)
	if err != nil {
		t.Fatalf("Failed to create xDS client: %v", err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), defaultTestTimeout)
	...
	_, closeFunc2, err := pool.NewClient(client2Name, &estats.UnimplementedMetricsRecorder{}, nil)
	if err != nil {
		t.Fatalf("Failed to create xDS client: %v", err)
	}
	name, err = clientImplCreateCh.Receive(ctx)
	...
	go func() {
		...
		for i := 0; i < count; i++ {
			var err error
			_, closeFuncs1[i], err = pool.NewClient(client1Name, &estats.UnimplementedMetricsRecorder{}, nil)
			if err != nil {
	...
	go func() {
		...
		for i := 0; i < count; i++ {
			var err error
			_, closeFuncs2[i], err = pool.NewClient(client2Name, &estats.UnimplementedMetricsRecorder{}, nil)
			if err != nil {
				t.Errorf("%d-th call to New() failed with error: %v", i, err)
			}
		}
